...
export class EmployeeService {
    ...
    async getEmployees(page: number = 1, limit: number = 10, search: string = '') {
        ...
        try {
            ...
            // ✅ CORREÇÃO: Construir query com LIMIT/OFFSET como valores literais
            // O mysql2 tem problemas com LIMIT ? e OFFSET ? em algumas versões
            query += ` ORDER BY u.name ASC LIMIT ${limitNum} OFFSET ${offset}`;
